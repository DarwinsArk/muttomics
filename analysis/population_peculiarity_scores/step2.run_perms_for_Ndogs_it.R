...
for (i in 1:nperm){
  stats <- sets %>% group_by(type,set,qf) %>% sample_n(ndogs) %>% summarise(mean=mean(value),n=length(value),i=i)   %>% select(type,set,qf,mean,n,i)
  write.table(stats,ofile,row.names=F,col.names=F,append=T,sep=",")

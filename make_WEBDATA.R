...
### make input datasets for website breed browser
...
quesOut <- paste("DarwinsArk_20191115_website.questions.csv",sep="")
factOut <- paste("DarwinsArk_20191115_website.factors.csv",sep="")
descOut <- paste("DarwinsArk_20191115_website.descriptions.csv",sep="")
...
ques <- ques %>% select(dog,qf,answer) %>% mutate(value=as.integer(answer)) %>% mutate(type="question") %>% select(-answer)

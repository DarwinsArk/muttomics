...
breeds_per_mutt <- dogs %>% filter(mutt) %>% inner_join(breed_calling_percents) 
...
write.csv(counts,"Fig_ANC_NBREEDS_MUTTS.histogram.csv",row.names=F)
...
write.csv(pd,"Fig_ANC_PROP_POP_MIN005.csv",row.names=F)
...
write.csv(pd,"Fig_ANC_DA_v_AKC.csv",row.names=F)

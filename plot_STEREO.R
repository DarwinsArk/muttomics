...
#%>% filter(!is.na(value)&str_length(value)>1) %>% rename(type=name) %>% rename(breed=value)
...
#okgrps <- d %>% ungroup() %>% select(qf,group,match) %>% distinct() %>% group_by(qf,group) %>% count() %>% filter(n>1) %>% select(-n)
#d <- d %>% inner_join(okgrps)
...
#ttest <- ttest %>% select(group,qf,plotname,change,group1,group2,n1,n2,meanYes,meanNo,statistic,df,p,p.adj,p.adj.signif,conf.low,conf.high)
write.csv(ttest,paste(outroot,".ttest.csv",sep=""),row.names = F)
...
d <- d %>% group_by(idtype,qf,trait) %>% summarize(nbreedsInc=n(),cor = cor(z,ranking,method="pearson"),p = cor.test(z,ranking,method="pearson")$p.value,conf.low = cor.test(z,ranking,method="pearson")$conf[1],conf.high = cor.test(z,ranking,method="pearson")$conf[2],n=n())  %>% adjust_pvalue(method="BH")
...
out <- paste("Fig_STEREO.encyclopedia.corr.data.csv",sep="")
write.csv((d %>% select(-longname)),out,row.names=F)

...
##dall <- dall %>% mutate(facet=if_else(anova=="breed+sex+age+size"|anova=="breed+sex+age","with breed in model","error")))
dall <- dall %>% mutate(facet=if_else(ds=="conf_breed","confirmed purebred dogs",if_else(ds=="cand_breed","candidate purebred dogs",if_else(ds=="all_dogs","all dogs",ds))))
...
#p <- p + geom_point(aes(color=sig),shape=16,size=1.25,alpha=0.5)
p <- p + geom_hline(yintercept=log2(1),color="grey20")

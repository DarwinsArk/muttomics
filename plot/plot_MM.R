...
ggsave(plot=p,filename="Fig_MM_OBS_EXP.pdf",width=6.5,height=4.5)

######### Make Fig_MM_OBS_EXP.main_text.pdf ######### 
comp_rand <- pd %>% filter(ratio>1) %>% group_by(strip) %>% count()
comp_rand <- pd %>% ungroup() %>% filter(ratio>1)  %>% count() %>% mutate(strip="all") %>% bind_rows(comp_rand)
comp_rand2 <- pd %>% group_by(strip) %>% count() %>% rename(ntot=n) 
comp_rand2 <- pd %>% ungroup() %>% count() %>% rename(ntot=n)  %>% mutate(strip="all") %>% bind_rows(comp_rand2)
comp_rand <- comp_rand %>% full_join(comp_rand2) 
comp_rand <- comp_rand %>% mutate(freq=n/ntot)
...
compare <- obs_by_dog_user %>% group_by(ProfDogTrainer) %>% summarize(n=n(),mean=mean(nright),sd=sd(nright))
ttest <- t.test(obs_by_dog_user %>% filter(ProfDogTrainer==0) %>% pull(nright),obs_by_dog_user %>% filter(ProfDogTrainer==1) %>% pull(nright))
print(compare)
print(ttest)
print(paste("p=",ttest$p.value,sep=""))

...
alld <- alld %>% mutate(set_w_size=if_else(lown,paste(set,"*",sep=""),set))
...
allxy <- pd %>% select(type,set,set_w_size,facetname) %>% distinct() %>% inner_join(allxy)
...
pd <- pd %>% mutate(ylabel=fct_reorder(ylabel,nrow))
empty <- empty %>% mutate(ylabel=fct_reorder(ylabel,nrow))
...
oksets <- pd  %>% filter(idtype=="factor"|id %in% c(121,125,127,17,54,59))
okqf <- oksets %>% select(idtype,id) %>% distinct()

# remove sets that have much missing data
oksets <- oksets %>% group_by(idtype,type,set) %>% summarize(nqf=n())
oksets <- oksets %>% filter(type!="by year")
oksets <- oksets %>% filter(idtype!="factor"|nqf>=7)
oksets <- oksets %>% filter(idtype!="question"|nqf==6)

# score sets based on whether they are popular breeds
oksetsPop <- oksets %>% select(-nqf) %>% inner_join(pd) %>% inner_join(okqf) %>% group_by(type,set) %>%  summarize(popsize=min(ndogs_available)) # %>% group_by(type,set) %>% summarize(top=max(abs(z)))
oksetsPop <- oksetsPop %>% arrange(desc(popsize)) %>% group_by(type) %>% mutate(nrow_pop=row_number()) %>% select(-popsize)

# score sets based on whether they have high scores for any behavior questions or factors (for more interesting main text figure)
oksetsScore <- oksets %>% select(-nqf) %>% inner_join(pd) %>% inner_join(okqf) %>% filter(id<120) %>% group_by(type,set) %>% summarize(top=max(abs(z)))
oksetsScore <- oksetsScore %>% arrange(desc(top)) %>% group_by(type) %>% mutate(nrow_score=row_number()) %>% select(-top)

# keep sets that are in the top 20 for score or popularity
oksets <- oksetsScore %>% inner_join(oksetsPop) %>% filter(nrow_score<20|nrow_pop<20) %>% select(type,set) %>% distinct()
oksets <- oksets %>% inner_join(pd) %>% inner_join(okqf)  %>% group_by(idtype,type,set) %>% count() %>% filter((idtype=="factor"&n>=7)|(idtype=="question"&n>=5)) %>% select(-n)



for (idtype_in in c("factor","question")){
  pd2 <- pd %>% inner_join(okqf) %>% inner_join(oksets) %>% filter(idtype==idtype_in)
...
p <- ggplot(pd2,aes(y=ylabel,x=set_w_size))
p <- p + geom_point(color="#525252",size=0.2)
...
p <- ggplot(pd3,aes(y=ylabel,x=set_w_size))
p <- p + geom_point(color="#525252",size=0.2)
...
p <- ggplot(pd4,aes(y=ylabel,x=set_w_size))
p <- p + geom_point(color="#525252",size=0.2)
...
p <- ggplot(pd5,aes(y=ylabel,x=set_w_size))
p <- p + geom_point(color="#525252",size=0.2)
...
### MAKE line plots comparing age and z score for all ages under 13
...
scales <- scales %>% mutate(y=if_else(name=="negative",-5,5)) %>% mutate(vjust=if_else(name=="negative",1,0))
...
p <- p + geom_text(aes(label=paste("R=",round(correlation,3),"; ",pstr,sep="")),color="black",size=1.6,lineheight=0.9,x=-1,y=6,hjust=0,vjust=0,data=pdcor) 
p <- p + geom_text(aes(y=y,label=value,vjust=vjust),hjust=0,size=1.6,x=-1,data=scales,color="grey30")
p <- p + geom_segment(x=-0.8,y=0,yend=4.75,xend=-0.8,arrow=arrow(length = unit(0.05, "inches")),color="grey30",size=0.3,alpha=0.5)
p <- p + geom_segment(x=-0.8,y=0,yend=-4.75,xend=-0.8,arrow=arrow(length = unit(0.05, "inches")),color="grey30",size=0.3,alpha=0.5)
p <- p + geom_hline(yintercept=0,color="grey30",size=0.1)
p <- p + scale_y_continuous("z score",limits=c(miny,maxy+1.1))
p <- p + scale_x_continuous("age (year)",breaks=c(0,2,4,6,8,10,12),limits=c(-1,12.5))

...
quesIn <- paste(indir,"DarwinsArk_20191115_answers.csv",sep="") 
raw <- as_tibble(read.csv(quesIn,header=T,na.strings=c("NA","#N/A","","N/A")))

quesInfo <- as_tibble(read.csv(paste(indir,"DarwinsArk_20191115_questions.csv",sep=""),header=T)) %>% select(id,short)
ans <- raw
dogs <- as_tibble(read.csv(paste(indir,"DarwinsArk_20191115_dogs.csv",sep=""),header=T))  %>% rename(dog=id)
...
write.csv(responses,"Fig_EXTREME_FRAC.data.csv",row.names=F)
